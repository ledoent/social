...
- Opening a single publication shows it read-only, with its account, its
  ...
  whose publication was deleted, has no address. The button asks the social
  media before opening anything, so a publication deleted there is reported
  as *The post does not exist or has been deleted.* and marked as *Deleted*
  right away, instead of waiting for the next synchronization. The card of
  the dashboard behaves the same way, because both ask the same question.
...
- The view shows one chart per connected account with the statistics of its
  publications. The statistics are requested to the social media when the
  view is opened, so a loader is displayed until they are available.
- The statistics can be grouped by day, by week or by month, and the period
  can be narrowed with the two dates. Each connector module offers only the
  granularities its own social media supports, so the list of the filter
  depends on the media of the account.
...
- When the credentials cannot be renewed at all, because the authorization was
  revoked or expired for good, the account is marked as needing an update: the
  warning appears on the dashboard and its responsible user is told in the
  chatter of the account. The publication is left as failed with the reason on
  it, and pressing *Post* again works once the account is authorized.
...
- Right after an account is linked, its publications and their statistics
  are imported: the scheduled action *Social: Initial sync of the new
  accounts* is triggered on the spot and the dashboard shows the account as
  syncing until it is over. The action also runs monthly, and it only
  processes the accounts still waiting for that first import.
- If that first import fails, the account stops waiting for it and is **not**
  ...
  again.
- The import also downloads the medias of the publications created outside
  of Odoo, so those images appear on the dashboard only after it.
- Afterwards the statistics are refreshed on demand, with the *Update*
  ...
  media updates* is what notices that a social media has something new every
  two hours, and it puts the warning on the dashboard that invites to press
  it. An account already announcing updates is left out of that check until
  the import clears the notice, so it costs nothing to leave it there.
- The *Update* button asks the social media only about what it needs, which is
  ...
  media: nothing is left to ask about. The scheduled action *Social: Full
  resync of the accounts* reads everything again once a week and reconciles it,
  and each connector may also offer to run it on demand from the account form.
  A publication deleted on the social media may therefore take up to a week to
  be reported as such.
...
- Accounts are recognised by the identifier they have on the social media,
  ...
  an existing one. The only exception are the old accounts that do not have
  that identifier stored yet: for them the user name is used as a fallback
