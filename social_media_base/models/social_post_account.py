...
class SocialPostAccount(models.Model):
    ...
    def _compute_interactions_count(self):
        for post_account in self:
            post_account.interactions_count = (
                post_account.click_count
                + post_account.like_count
                + post_account.share_count
                + post_account.comment_count
            )
    ...
    def action_open_post_account_url(self):
        ...
        The address is only known for a publication that made it to the
        social media, so the button showing it is hidden otherwise. The address
        alone is not proof that the publication is still online: it survives
        a deletion made on the social media until the updates cron notices,
        so the social media is asked before the address is opened.
        """
        self.ensure_one()
    ...
    def action_like_comment(self, comment_id=None, author_urn=None):
        ...
        :param comment_id: identifier of the comment on the social media.
        :param author_urn: actor urn performing the like.
        :return: ``success`` and ``message`` of the action.
        :rtype: dict
        """
