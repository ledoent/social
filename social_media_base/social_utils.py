...
def social_url_encode(
    ...
    if isinstance(params_values[param_field], list):
        values = (
            "List("
            + ",".join(
                quote(param_value, safe=",")
                for param_value in params_values[param_field]
            )
            + ")"
        )
        url_format = f"{param_field}={url_quote(values, safe='()%,')}".replace("+", "")
    elif format_quote:
        url_format = (
            f"{param_field}={url_quote(params_values[param_field], safe='()%,')}"
        )
    else:
...
def get_weeks(start_date, end_date, freq="W-MON"):
    ...
    if freq == "D":
        ...
        # Align start_date to the next Monday
