...
class TestConnectionHealth(TestSocialMediaBaseCommon):
    ...
    def test_cron_swallows_per_account_errors(self):
        # The cron deliberately logs `_logger.exception(...)` for each
        # bad account; mute the social_account logger so OCA's checklog
        # CI step doesn't flag the expected error lines.
        b = self.SocialAccount.create(
            {"name": "Second", "media_id": self.social_media_id.id}
        )
    ...
    def test_post_health_warning_replaces_prior_reconnect_activity(self):
        """A warning → healthy → warning cycle should leave exactly one
        open reconnect activity (the latest), not pile up stale rows.
        """
        a = self.social_account_id
        a.health_status = "warning"
        # First degradation creates an activity
        a._post_health_warning(prior_status="healthy")
        first_count = self.env["mail.activity"].search_count(
            [("res_model", "=", "social.account"), ("res_id", "=", a.id)]
        )
        self.assertEqual(first_count, 1)
        # Account recovered, then degraded again — second call should
        # drop the stale activity and create exactly one fresh one
        a._post_health_warning(prior_status="healthy")
        second_count = self.env["mail.activity"].search_count(
            [("res_model", "=", "social.account"), ("res_id", "=", a.id)]
        )
        self.assertEqual(
            second_count,
            1,
            "second degradation should replace, not duplicate",
        )

    def test_post_health_warning_summary_contains_reconnect(self):
        a = self.social_account_id
        a.health_status = "expired"
        a._post_health_warning(prior_status="healthy")
        ...
        self.assertIn("Reconnect", activity.summary)
    ...
    # ── Wizard pre-flight fallback ───────────────────────────────────
    def test_preflight_fallback_for_unknown_media_type(self):
        """An unsupported media_type renders the neutral fallback copy
        and hides the devportal CTA — never misrepresents the platform
        as LinkedIn-branded.
        """
        other_media = self.SocialMedia.create({"name": "Other"})
        wizard = self.WizardAccount.create({"media_id": other_media.id})
        copy = wizard._preflight_copy()
        self.assertEqual(copy["headline"], "Connect your account")
        self.assertEqual(copy["devportal_url"], "")
        self.assertFalse(wizard.preflight_has_devportal)
        self.assertIn("Connect your account", wizard.preflight_html)
        # Logo tag is omitted entirely when no asset is registered
        self.assertNotIn("<img", wizard.preflight_html)
