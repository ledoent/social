...
# out. Taken from a real account.
RECENT_STATISTICS_LINKEDIN = {
    "2026-08-06": (0, 0, 0, 0, 17),
    "2026-08-07": (5, 0, 0, 0, 29),
    "2026-08-08": (0, 1, 0, 0, 0),
}
...
class TestSocialLinkedin(TestSocialCommonLinkedin):
    ...
    def _generate_update_posts_statistics_patches(
        ...
        return (
            ...
            self.generate_patch(
                **{
                    "model_patch": PATCH_ACCOUNT_LINKEDIN.format(
                        "_get_linkedin_recent_statistics"
                    ),
    ...
    def test_update_posts_statistics_marks_the_page(self):
        ...
        self.assertEqual(
            ...
            _linkedin_statistics_checkpoint(RECENT_STATISTICS_LINKEDIN),
        )
    ...
    def test_chart_granularities_drop_the_week(self):
        """LinkedIn only groups the share statistics by day or by month."""
        self.assertEqual(
            self.SocialAccountLinkedin._get_chart_granularities(),
            ["DAY", "MONTH"],
        )

    def _patch_chart_statistics(self, elements):
        """Patch the finder answering one bucket per period."""
        return (
            self.generate_patch(
                **{
                    "type_object": True,
    ...
    def _chart_bucket(self, start, **statistics):
        return {
    ...
    def test_get_chart_account_statistics(self):
        """Every bucket is charted on its own day, never on its position."""
        patch_dates, patch_request = self._patch_chart_statistics(
            [
                # 2025-01-02 and 2025-01-05, in milliseconds.
                self._chart_bucket(
                    1735776000000,
                    clickCount=100,
                    likeCount=30,
        ...
        patch_totals = self.generate_patch(
            **{
                "type_object": True,
                "model_patch": self.SocialAccountLinkedin,
                "method_patch": "_get_linkedin_chart_totals",
                "return_value": {
                    "impressionCount": 280,
                    "commentCount": 3,
                    "reactionCount": 9,
        ...
        with patch_dates, patch_request, patch_totals:
            result = self.SocialAccountLinkedin._get_chart_account_statistics(
                start_date="2025-01-01", end_date="2025-01-07", granularity="DAY"
            )
        ...
        self.assertEqual(chart["datasets"][0]["data"], [0, 100, 0, 0, 200, 0, 0])
        self.assertEqual(chart["datasets"][1]["data"], [0, 0, 0, 0, 1, 0, 0])
        self.assertEqual(chart["datasets"][2]["data"], [0, 30, 0, 0, 70, 0, 0])
        self.assertEqual(chart["datasets"][3]["data"], [0, 50, 0, 0, 100, 0, 0])
        self.assertEqual(chart["datasets"][4]["data"], [0, 7, 0, 0, 9, 0, 0])
        self.assertEqual(chart["datasets"][5]["data"], [0, 0, 0, 0, 0.5, 0, 0])
        self.assertEqual(chart["impressionCount"], 16, "The ones of the period.")
        self.assertEqual(
            chart["impressionCountTotal"], 280, "The ones of the whole account."
        )
        self.assertEqual(chart["commentCountTotal"], 3)
        self.assertEqual(chart["reactionCountTotal"], 9)

    def test_get_chart_account_statistics_without_totals(self):
        """Filtering does not read the figures of the account again."""
        patch_dates, patch_request = self._patch_chart_statistics(
            [self._chart_bucket(1735776000000, impressionCount=7)]
        )
        patch_totals = self.generate_patch(
            **{
                "type_object": True,
                "model_patch": self.SocialAccountLinkedin,
                "method_patch": "_get_linkedin_chart_totals",
                "return_value": {"impressionCount": 280},
            }
        )
        with patch_dates, patch_request, patch_totals as mock_totals:
            result = self.SocialAccountLinkedin._get_chart_account_statistics(
        ...
        self.assertIsNone(result[0]["impressionCountTotal"])

    def test_get_linkedin_chart_totals(self):
        """The figures of the account are the series over the whole history.

        Reading them per post would mix the totals of the posts Odoo knows
        with the ones of the whole organization page, and the figure of a
        period would not be a part of them.
        """
        _patch_dates, patch_request = self._patch_chart_statistics(
            [
                self._chart_bucket(
                    1735776000000, likeCount=4, commentCount=1, impressionCount=100
                ),
                self._chart_bucket(
                    1736035200000,
                    likeCount=5,
        ...
        with patch_request as mock_request:
            totals = self.SocialAccountLinkedin._get_linkedin_chart_totals()
        self.assertEqual(
            totals,
            {"impressionCount": 280, "commentCount": 3, "reactionCount": 10},
        )
        self.assertIn(
            "timeGranularityType:MONTH",
            mock_request.call_args.kwargs["params_values"]["timeIntervals"],
            msg="The history is asked for by month, not day by day.",
        )

    def test_get_linkedin_chart_statistics_adds_up_the_same_period(self):
        """Two buckets of one period are added up, not overwritten."""
        _patch_dates, patch_request = self._patch_chart_statistics(
            [
                # Both are days of january, so both are the "2025-01" month.
                self._chart_bucket(1735776000000, impressionCount=100),
                self._chart_bucket(1736035200000, impressionCount=180),
            ]
        )
        with patch_request:
            statistics = self.SocialAccountLinkedin._get_linkedin_chart_statistics(
                0, 0, "MONTH", "ME"
            )
        self.assertEqual(list(statistics), ["2025-01"])
        self.assertEqual(statistics["2025-01"][5], 280)

    def test_get_chart_account_statistics_error(self):
        patch_dates = self._patch_chart_statistics([])[0]
        patch_request_failed = self.generate_patch(
            **{
                "type_object": True,
                "model_patch": self.SocialAccountLinkedin,
                "method_patch": "_request_linkedin",
        ...
        with patch_dates, patch_request_failed, self.assertRaises(UserError):
            self.SocialAccountLinkedin._get_chart_account_statistics(
                start_date="2025-01-01", end_date="2025-01-07", granularity="DAY"
            )
    ...
    def test_get_default_filter_date(self):
        ...
        self.assertEqual(
            result,
            generate_timestamps(date_start=start_date, date_end=end_date),
    ...
    def _linkedin_query_string(self, call):
        ...
        return "&".join(
            social_url_encode(
                param_field,
                kwargs["params_values"],
                kwargs.get("params_values_char_ignore"),
                kwargs.get("format_quote"),
            )
    ...
    def _mark_the_page_as_imported(self, accounts=None, statistics=None):
        ...
        accounts.linkedin_statistics_checkpoint = _linkedin_statistics_checkpoint(
    ...
    def _patch_recent_statistics(self, statistics=None, side_effect=None):
        if side_effect:
            return patch(
                PATCH_ACCOUNT_LINKEDIN.format("_get_linkedin_recent_statistics"),
                autospec=True,
        ...
        return patch(
            ...
            autospec=True,
            return_value=dict(
                RECENT_STATISTICS_LINKEDIN if statistics is None else statistics
            ),
        )

    def test_get_linkedin_recent_statistics(self):
        """The daily buckets are asked for, and the engagement is dropped."""
        buckets = {
            "2026-08-07": (5, 0, 0, 0, 0.17, 29),
            "2026-08-08": (0, 1, 0, 0, 0.0, 0),
        }
        with patch(
            PATCH_ACCOUNT_LINKEDIN.format("_get_linkedin_chart_statistics"),
            autospec=True,
            return_value=buckets,
        ) as mock_chart:
            statistics = self.SocialAccountLinkedin._get_linkedin_recent_statistics()
        self.assertEqual(
            statistics,
            {"2026-08-07": (5, 0, 0, 0, 29), "2026-08-08": (0, 1, 0, 0, 0)},
            msg="The engagement is a ratio of the others, so it is not watched.",
        )
        start_time, end_time, granularity, freq = mock_chart.call_args.args[1:]
        self.assertEqual(granularity, "DAY")
        self.assertEqual(freq, "D")
        self.assertEqual(
            round((end_time - start_time) / (24 * 3600 * 1000)),
            _UPDATE_CHECK_DAYS_LINKEDIN,
            msg="The window has to be wider than the interval of the cron.",
        )
        self.assertGreater(
            end_time,
            generate_timestamps(date_start=fields.Datetime.now())[0],
    ...
    def test_get_linkedin_recent_statistics_without_figures(self):
        with patch(
            PATCH_ACCOUNT_LINKEDIN.format("_get_linkedin_chart_statistics"),
            autospec=True,
            return_value={},
            ...
            self.assertEqual(
                self.SocialAccountLinkedin._get_linkedin_recent_statistics(), {}
            )
    ...
    def test_run_check_media_updates_when_the_page_moved(self):
        ...
        moved = {**RECENT_STATISTICS_LINKEDIN, "2026-08-08": (0, 2, 0, 0, 0)}
    ...
    def test_run_check_media_updates_notices_the_impressions_alone(self):
        ...
        self._isolate_linkedin_account()
        self._mark_the_page_as_imported()
        moved = {**RECENT_STATISTICS_LINKEDIN, "2026-08-07": (5, 0, 0, 0, 30)}
    ...
    def test_run_check_media_updates_notices_a_new_day_with_activity(self):
        ...
        self._isolate_linkedin_account()
        self._mark_the_page_as_imported()
        moved = {**RECENT_STATISTICS_LINKEDIN, "2026-08-09": (0, 1, 0, 0, 0)}
        with self._patch_recent_statistics(moved):
            self.SocialAccount._run_check_media_updates()
        self.assertTrue(self.SocialAccountLinkedin.need_update)
    ...
    def test_run_check_media_updates_ignores_an_empty_new_day(self):
        ...
        self._isolate_linkedin_account()
        self._mark_the_page_as_imported()
        quiet = {**RECENT_STATISTICS_LINKEDIN, "2026-08-09": (0, 0, 0, 0, 0)}
    ...
    def test_run_check_media_updates_skips_a_flagged_account(self):
        """An account already announcing updates costs no call at all."""
        self._isolate_linkedin_account()
        self._mark_the_page_as_imported()
    ...
    def test_run_check_media_updates_isolates_each_account(self):
        ...
        def recent_statistics(account):
            ...
            return {**RECENT_STATISTICS_LINKEDIN, "2026-08-08": (0, 2, 0, 0, 0)}
