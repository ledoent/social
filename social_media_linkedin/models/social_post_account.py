        ...
        if not advertising_account_id:
            ...
            if response.status_code == 200:
                ...
                raise ValidationError(
                    self.env._(
                        "Error get advertising account in Linkedin: %s",
                        response.json(),
                    )
                )
        ...
        if campaign_group_linkedin_urn:
            ...
            if campaign and campaign.status_code == 200:
                ...
                if response.status_code == 201:
                    ...
                    raise ValidationError(
                        self.env._(
                            "Error creating campaign in Linkedin: %s",
                            response.json(),
                        )
                    )
            else:
                raise ValidationError(
                    self.env._(
                        """Error creating group campaign in Linkedin:
                        %s""",
                        campaign.json(),
                    )
                )
        ...
        if (
            ...
            if campaign_linkedin_urn and post_id:
                ...
                if response.status_code == 201:
                    ...
                    raise ValidationError(
                        self.env._(
                            """Error creating campaign post in Linkedin:
                            %s""",
                            response.json(),
                        )
                    )
