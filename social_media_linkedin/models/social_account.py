...
from ..social_linkedin_utils import (
    _CHART_HISTORY_MONTHS_LINKEDIN,
...
class SocialAccount(models.Model):
    ...
    def _get_entity_share_statistics(
        ...
        fixed_bytes = self._query_string_bytes(
            params_fields, params_values, params_values_char_ignore, format_quote
        )
    ...
    def _get_linkedin_recent_statistics(self):
        """Return the figures LinkedIn reports for the whole page, day by day.
        ...
        The range ends tomorrow, not now: LinkedIn takes the end of a time
        interval as exclusive and normalizes it to the day, so asking up to the
        current moment leaves the bucket of today out of the answer, which is
        the one bucket the check is really after.
        ...
        start_date = end_date - timedelta(days=_UPDATE_CHECK_DAYS_LINKEDIN)
        start_time, end_time = self._get_default_filter_date(
            start_date, end_date, time_date=True
        )
        buckets = self._get_linkedin_chart_statistics(start_time, end_time, "DAY", "D")
        return {
            period: tuple(figures[index] for index in _UPDATE_CHECK_FIGURES_LINKEDIN)
            for period, figures in buckets.items()
        }
    ...
    def _refresh_linkedin_posts(self, post_id=None, full_feed=False):
        ...
        for ugc_post in ugc_posts:
            ...
            data = {
                ...
                "published_date": convert_to_date(
                    milliseconds=ugc_post.get("publishedAt")
        ...
        if not post_id:
            ...
            ] = _linkedin_statistics_checkpoint(self._get_linkedin_recent_statistics())
    ...
    def _get_chart_granularities(self):
        """Drop the week, LinkedIn does not group its statistics by week.

        ``organizationalEntityShareStatistics`` documents ``DAY`` and
        ``MONTH`` as the only values of ``timeGranularityType``.
        """
        granularities = super()._get_chart_granularities()
        if all(account.media_type == "linkedin" for account in self):
            granularities = [
                granularity for granularity in granularities if granularity != "WEEK"
            ]
        return granularities

    def _get_linkedin_chart_statistics(self, start_time, end_time, granularity, freq):
        """Return the statistics of this account by chart period.
        ...
        are keyed by period instead of being returned as a plain list.
        ...
        for element in response.json().get("elements", []):
            ...
            period = get_chart_period_key(
                datetime.fromtimestamp(bucket_start / 1000), freq
            )
            bucket = (
            ...
            # Several buckets of the same period are added up instead of
            # overwriting each other: LinkedIn answers one per granularity
            # unit, but a period of the chart may be wider than that unit.
            previous = statistics.get(period)
            statistics[period] = (
                bucket
                if previous is None
                else tuple(
    ...
    def _get_linkedin_chart_totals(self):
        """Return the figures of the whole history, not of a period.

        They are read from the same finder as the series, over the months
        LinkedIn keeps, so the figure of the period is really a part of this
        one. Reading them per post would mix two different measures: the
        finder reports the whole organization page, including what was
        published before Odoo managed the account or outside of it, while a
        post only knows its own lifetime totals.

        :rtype: dict
        """
        start_time, end_time = self._get_default_filter_date(
            None, None, time_date=True, months=_CHART_HISTORY_MONTHS_LINKEDIN
        )
        statistics = self._get_linkedin_chart_statistics(
            start_time, end_time, "MONTH", "ME"
        ).values()
        return {
            "impressionCount": sum(period[5] for period in statistics),
            "commentCount": sum(period[2] for period in statistics),
            "reactionCount": sum(period[1] + period[3] for period in statistics),
        }

    def _get_chart_account_statistics(
        self, start_date=None, end_date=None, granularity="DAY", with_totals=True
    ):
        data = super()._get_chart_account_statistics(
        ...
        if not self:
            ...
            return data
        data_linkedin = []
        freq = "ME" if granularity == "MONTH" else "D"
        for account in account_ids:
            start_time, end_time = account._get_default_filter_date(
                start_date, end_date, time_date=True
            )
            period_start, period_end = account._get_default_filter_date(
                start_date, end_date
            )
            statistics = account._get_linkedin_chart_statistics(
                start_time, end_time, granularity, freq
            )
            if statistics:
                data_linkedin += account._map_chart_statistics(
                    ...
                    end_date=period_end,
                    totals=account._get_linkedin_chart_totals()
                    if with_totals
                    else None,
                )
        ...
        try:
            ...
            for account in account_ids:
                try:
                    ...
                    with self.env.cr.savepoint():
                        update = account._check_linkedin_updates() or update
                except psycopg2.OperationalError as error:
                    if error.pgcode in PG_CONCURRENCY_ERRORS_TO_RETRY:
                        raise
                    _logger.exception(
    ...
    def _check_linkedin_updates(self):
        ...
        """
        self.ensure_one()
        statistics = self._get_linkedin_recent_statistics()
        previous = _linkedin_statistics_snapshot(self.linkedin_statistics_checkpoint)
        if not previous:
            ...
            self.linkedin_statistics_checkpoint = _linkedin_statistics_checkpoint(
                statistics
            )
            return False
        if _linkedin_statistics_moved(previous, statistics):
